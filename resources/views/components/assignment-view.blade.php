...
@isset($parentItemName)

<p class="text-xl text-gray-900 dark:text-white">{{ucfirst($parentItemName)}} : {{$parentItem->name}}</p>
        @if($itemName=='skillcategory')

            {{ucfirst($itemName)}}

        @else
            {{ucfirst(Str::plural($itemName))}}
        ...
        <tbody>
            ...
            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                @isset($pathDelete)
                    ...
                    <form method="POST" action="{{route($pathDelete,[$parentItemName=>$parentItem,$itemName=>$item])}}">
                        @csrf
                        @method('DELETE')
                        @if($parentItemName=="skillcategory")

                            <button type="submit" class="btn_red">Delete</button>

                        @else

                            <button type="submit" class="btn_purple">Unassign</button>
                ...
                <div>
                    ...
                    <div id="alert-border-1" class="flex items-center p-4 mb-4 text-blue-800 border-t-4 border-blue-300 bg-blue-50 dark:text-blue-400 dark:bg-gray-800 dark:border-blue-800" role="alert">
                        ...
                        <div class="ms-3 text-sm font-medium">
                            There are no {{str::plural($itemName)}} to show!.
                         </div>
