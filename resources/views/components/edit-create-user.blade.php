...
@auth('sysadmin')
    <form method="POST"
        action="{{ isset($institute) ? route('sysadmin.institutes.update', $institute) : route('sysadmin.institutes.store') }}"
        class="max-w-sm mx-auto">
    @elseif ('institute')
        <form method="POST"
            action="{{ isset($institute) ? route('institutes.update', $institute) : route('institutes.store') }}"
            class="max-w-sm mx-auto">
        @endauth
        ...
        <div class="flex flex-col mt-16">
            <div class="mb-5">
                @if ($errors->has('name'))
                    <label for="name"
                        class="block mb-2 text-sm font-medium text-red-700 dark:text-red-500">Name</label>
                    <input type="text" id="name" name="name"
                        class="bg-red-50 border border-red-500 text-red-900
            ...
            <div class="mb-5">
                ...
                @if ($errors->has('email'))
                    ...
                    <input type="email" id="email" name="email"
                        class="bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-sm rounded-lg focus:ring-red-500
            ...
            <div class="mb-5">
                ...
                @if ($errors->has('address'))
                    <label for="address"
                        class="block mb-2 text-sm font-medium text-red-700 dark:text-red-500">Address</label>
            ...
            <div class="flex justify-around">
                ...
                @isset($institute)
                    @auth('sysadmin')
                        <a href="{{ route('sysadmin.institutes.password_change_create', $institute) }}"
                            class="btn_red">Password</a>
                    @elseif('institute')
                ...
                @isset($institute)
                    @auth('sysadmin')
                        <a href="{{ route('sysadmin.institutes.email_change_create', $institute) }}"
                            class="btn_yellow ">Email</a>
                    @elseif ('institute')
